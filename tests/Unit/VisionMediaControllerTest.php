...
use Zephyrisle\FlarumZaiBot\Api\Controller\VisionMediaController;

class VisionMediaControllerTest extends TestCase
{
    ...
    {
        $this->tmpDir = sys_get_temp_dir() . '/zai-bot-media-' . uniqid();
    ...
    protected function controller(
        ...
        return new VisionMediaController(
            $paths,
    ...
    public function testReturns404ForNonMediaType(): void
    {
    ...
    public function testReturns404ForOversizedImageFile(): void
    {
        // 图片超过 20MB 上限 → 拒绝代理
    ...
    public function testServesVideoFile(): void
    {
        file_put_contents($this->tmpDir . '/assets/files/uploads/photo.png', 'FAKE_MP4_BYTES');
        $this->makeFile(['type' => 'video/mp4', 'size' => strlen('FAKE_MP4_BYTES')]);

        $response = $this->controller()->handle($this->request());

        $this->assertSame(200, $response->getStatusCode());
        $this->assertSame('FAKE_MP4_BYTES', (string) $response->getBody());
        $this->assertSame('video/mp4', $response->getHeaderLine('Content-Type'));
    }

    public function testServesAudioFile(): void
    {
        file_put_contents($this->tmpDir . '/assets/files/uploads/photo.png', 'FAKE_WAV_BYTES');
        $this->makeFile(['type' => 'audio/wav', 'size' => strlen('FAKE_WAV_BYTES')]);

        $response = $this->controller()->handle($this->request());

        $this->assertSame(200, $response->getStatusCode());
        $this->assertSame('FAKE_WAV_BYTES', (string) $response->getBody());
        $this->assertSame('audio/wav', $response->getHeaderLine('Content-Type'));
    }

    public function testVideoHasLargerSizeLimit(): void
    {
        // 视频允许更大文件（300MB 上限），这里测试 50MB 的视频应放行
        file_put_contents($this->tmpDir . '/assets/files/uploads/photo.png', 'LARGE');
        $this->makeFile(['type' => 'video/mp4', 'size' => 50 * 1024 * 1024]);
    ...
    public function testAudioHasLargerSizeLimit(): void
    {
        // 音频允许 100MB 上限，这里测试 30MB 的音频应放行
        file_put_contents($this->tmpDir . '/assets/files/uploads/photo.png', 'LARGE');
        $this->makeFile(['type' => 'audio/wav', 'size' => 30 * 1024 * 1024]);
